    ...
    while (!m_shouldStop)
    {
        ...
        if (!blocks.empty())
        {
...

            {
                ...
                m_haveProcessedBlocksToHandle.wait(
                    lock,
                    [&]
                    {
                        if (m_shouldStop)
    ...
    const std::vector<std::tuple<Crypto::PublicKey, WalletTypes::TransactionInput>> &inputs) const
{
    ...
    std::copy_if(
        inputs.begin(),
        inputs.end(),
        std::back_inserter(relevantInputs),
        [&](const auto input) { return std::get<1>(input).parentTransactionHash == tx.hash; });
