...
namespace Utilities
{
    template<typename ReturnValue> class ThreadPool
    {
      public:
        /////////////////
        /* CONSTRUCTOR */
        ...
        ThreadPool(uint64_t threadCount): m_shouldStop(false)
        {
            if (threadCount == 0)
            {
                threadCount = 1;
      ...
      private:
        ...
        std::queue<std::tuple<std::function<ReturnValue()>, std::promise<ReturnValue>>> m_queue;
    };
} // namespace Utilities
