...
namespace Utilities
{
    ...
    ParsedExtra parseExtra(const std::vector<uint8_t> &extra)
    {
        ...
        for (auto it = extra.begin(); it < extra.end(); it++)
        {
            ...
            if (seenPubKey && seenPaymentID && seenMergedMiningTag && seenExtraData && seenRecipientPublicViewKey
                && seenRecipientPublicSpendKey && seenTransactionPrivateKey && seenPoolNonce)
            {
                break;
            }
            ...
            */
            ...
            if (c == Constants::TX_EXTRA_POOL_NONCE && elementsRemaining > 1 && !seenPoolNonce)
            {
                ...
                if (elementsRemaining > readNonceSize + nonceSize)
                {
                    ...
                    std::copy(
                        it + 1 + readNonceSize,
                        it + 1 + readNonceSize + nonceSize,
                        std::back_inserter(parsed.poolNonce));
